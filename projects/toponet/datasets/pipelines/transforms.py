...
#                 dtype=np.float32,
#             )
#             results['scale_factor'] = scale_factor * results[
#                 'scale_factor'] if 'scale_factor' in results else scale_factor
...
class CustomParametrizeSDMapGraph:
    ...
    def points_all_with_type(self, sd_map):
            ...
            if len(polylines) > 0:
                for segment in polylines:
                    ...
                    sd_map_graph.append(np.concatenate([segment.astype(np.float32),
                                                        np.ones([segment.shape[0], 1], dtype=np.float32) *
                                                        self.category2id[category]],
                                                       axis=1))
        ...
        # pad at the end
        sd_map_graph = [np.pad(map_graph, ((0, max_num_points - map_graph.shape[0]), (0, 0)), 'constant') for map_graph
                        in sd_map_graph]
        # list of segments: num_polylines, max_num_points x 3

        # then stack: num_polylines x max_num_points x 3
        sd_map_graph = np.stack(sd_map_graph, axis=0) if len(sd_map_graph) > 0 else np.zeros([0, max_num_points, 3],
                                                                                             dtype=np.float32)
    ...
    def points_onehot_type(self, sd_map):
        ...
        for category, polylines in sd_map.items():
            ...
            if len(polylines) > 0:
                for segment in polylines:
                    ...
                    sd_map_graph.append(np.concatenate([segment.astype(np.float32),
                                                        lane_category_onehot.reshape(1, -1).repeat(segment.shape[0],
                                                                                                   axis=0)],
                                                       axis=1))
        if len(num_pnt_per_segment) == 0:
            num_pnt_per_segment = np.empty([0], dtype=np.int64)
        ...
        sd_map_graph = np.stack(sd_map_graph, axis=0) if len(sd_map_graph) > 0 else np.zeros(
            [0, max_num_points, num_categories + 2], dtype=np.float32)
        map_meta = dict(map_num_poly_pnts=num_pnt_per_segment)
        return sd_map_graph, map_meta
