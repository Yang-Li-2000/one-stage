...
class SubgraphNet_Layer(BaseModule):
    ...
    def forward(self, input, num_vec_per_polyline):
        ...
        polyline_feature = max_pool_max_indices(embedd_data,
                                                num_vec_per_polyline)  # (num_polylines, 1, hidden_channels)
        ...
        polyline_feature = polyline_feature.repeat(1, max_num_vectors,
                                                   1)  # (num_polylines, max_num_vectors, hidden_channels)
        output = torch.cat([embedd_data, polyline_feature],
                           dim=-1)  # (num_polylines, max_num_vectors, hidden_channels*2)
        return output
