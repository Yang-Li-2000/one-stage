...
class SoftPolygonFunction(Function):
    @staticmethod
    def forward(ctx, vertices, width, height, inv_smoothness=1.0, mode=MODE_BOUNDARY, rasterized_dtype=None):
        ctx.width = width
        ctx.height = height
        ...
        if rasterized_dtype is None:
            ...
            rasterized = torch.DoubleTensor(ctx.batch_size, ctx.height, ctx.width).fill_(0.0).to(device=ctx.device)
...
class SoftPolygon(nn.Module):
    ...
    def forward(self, vertices, width, height, p, color=False, rasterized_dtype=None):
        return SoftPolygonFunction.apply(vertices, width, height, self.inv_smoothness, self.mode, rasterized_dtype)
